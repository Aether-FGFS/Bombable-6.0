...
## Revision : Aether, GPT, Claude 11/04/2026,14/05/2026
###################################################################################
var bombableVersion = "6.0.2";
##
...
#
# Required:
#  1. Bombable-6.0, must be added as Add-ons Module into Flightgear and 
#     Bombable-Sounds folder should be copied under your fgfs folder :
#     fgdata/AI/Aircraft/Bombable-Sounds
# 
#
...
#
# Notes: How to add Sounds into AI model. 
#   -If we do not need to use other sounds for AI model, we can use our 
#    bombable-weapons-sound.xml at relative path located under folder
#    bombable-6.0/FGData/AI/Aircraft/Bombable-Sounds with relative path
#    "AI/Aircraft/Bombable-Sounds" to add weapons sounds into model direct trough
#    some-sound.xml located under fgdata/AI/Aircraft/model-name/Sounds folder. 
#   -To have different sounds.. engine and other you must use sound under
#    flightgear installation AI/Aircraft/Bombable-Sounds.Simlpy copy file
#    bombable-weapons-sound.xml , rename it to ai-sound.xml and paste it 
#    under AI/Aircraft/your model/Sounds folder. Add your sounds into ai-sound.xml
#    do not forget to register new ai-sound.xml inside model.xml to play sounds
#    and you are done. Bombable will play sounds from inside of your AI Sounds. 
#
# AUTHORS
...
var dialog = {
  ...
  create : func {
    ...
    foreach (var b; [["Bombable module enabled", bomb_menu_pp~"bombable-enabled", "checkbox"],
                         ...
                         ["Allow AI vs AI attack (by faction)", bomb_menu_pp~"ai-vs-ai-enabled", "checkbox"],
                         ["Max AI attacking player (0=all)", bomb_menu_pp~"max-ai-attack-player", "slider", 200, 0, 20],
                         ["", "", "hrule"],
                         ...
                         ["AI Weapon effectiveness (AI aircraft's weapons)", bomb_menu_pp~"ai-weapon-power-combo", "combo", 300, 
                         ["Much more effective", "More effective", "Normal", "Less effective", "Much less effective", "Disabled (they can't shoot at you)"]],
                         #["AI fighter aircraft maneuver and attack", bomb_menu_pp~"ai-aircraft-attack-enabled", "checkbox"],
                         ["AI aircraft flying/dogfighting skill", bomb_menu_pp~"ai-aircraft-skill-combo", "combo", 300, 
                         ["Very skilled", "Above average", "Normal", "Below average", "Unskilled", "Disabled (AI aircraft can't maneuver)"]],
...
var init_bombable_dialog_listeners = func {
  ...
  setlistener(""~bomb_menu_pp~"ai-aircraft-skill-combo", func {
    ...
    # "Realistic", "Easy", "Super Easy", "Super-Duper Easy"
    # max-ai-attack-player is now a standalone menu slider
    if ( val == "Very skilled" ) {
...
var setupBombableMenu = func {
  ...
  if ( getprop(bomb_menu_pp~"ai-vs-ai-enabled") == nil ) {
    props.globals.getNode(bomb_menu_pp~"ai-vs-ai-enabled", 1).setBoolValue(0);
  }

  # max number of AI aircraft that will simultaneously attack the player (0 = unlimited)
  if ( getprop(bomb_menu_pp~"max-ai-attack-player") == nil ) {
    props.globals.getNode(bomb_menu_pp~"max-ai-attack-player", 1).setIntValue(3);
  }
  ...
  if ( getprop("/bombable/menusettings/fire-particles/flack-trigger") == nil ) {
    props.globals.getNode("/bombable/menusettings/fire-particles/flack-trigger", 1).setBoolValue(1);
  }
...
var getBallisticMass_lb = func (impactNodeName) {
  ...
  if (ballisticMass_slug != nil ) {
    ...
    } elsif (find("76mm", impactType ) != -1 ) {
      ballisticMass_lb = 13.5;
...
# Draw flack-impact visual on player aircraft when hit by weapons
    if ( source == "weapons" and getprop(bomb_menu_pp~"fire-particles/flack-trigger") ) {
      var _plat = getprop("/position/latitude-deg");
      var _plon = getprop("/position/longitude-deg");
      var _palt = getprop("/position/altitude-ft") * FT2M;
      if ( _plat != nil and _plon != nil and _palt != nil ) {
        debprint("Bombable: Drawing flack-impact on player at ", _plat, " ", _plon, " ", _palt);
        var _offset = 3;
        var _dlat = (rand() - 0.5) * 2 * _offset / 111320;
        var _dlon = (rand() - 0.5) * 2 * _offset / 111320;
        var _dalt = rand() * _offset;
        put_remove_model( _plat + _dlat, _plon + _dlon, _palt + _dalt, 0.75, 0.3, 1.5,
                          "AI/Aircraft/Fire-Particles/smallflack.xml" );
      }
    }
...
# turned on.
#
var fireAIWeapon = func (time_sec=1, myNodeName="", weaponTrigger="bombable/ambient-trigger-mg") {
  setprop(""~myNodeName~"/bombable/fire-particles/ai-weapon-firing",1);
  
  # Ambient sound toggle - per weapon type
  var _acur = getprop(myNodeName ~ "/" ~ weaponTrigger) or 0;
  setprop(myNodeName ~ "/" ~ weaponTrigger, !_acur);
...
var weapons_loop = func (id, myNodeName1="", myNodeName2="", targetSize_m=nil) {
 ...
 # Burst pattern - short fire
  var _firing = getprop(myNodeName1 ~ "/bombable/weapons-firing") or 0;
  var _burstCount = getprop(myNodeName1 ~ "/bombable/weapons-burst-count") or 0;

  if (_firing) {
    # Burst mode - fast repeating
    _burstCount += 1;
    setprop(myNodeName1 ~ "/bombable/weapons-burst-count", _burstCount);
    if (_burstCount > 3 + int(rand()*4)) {
      # End of burst -  pause 1-5 seconds
      setprop(myNodeName1 ~ "/bombable/weapons-firing", 0);
      setprop(myNodeName1 ~ "/bombable/weapons-burst-count", 0);
      var loopLength = 1.0 + rand() * 4.0;
    } else {
      var loopLength = 0.1 + rand() * 0.2;
    }
  } else {
    # Outside burst - Set new burst
    if (rand() < 0.3) {
      setprop(myNodeName1 ~ "/bombable/weapons-firing", 1);
      setprop(myNodeName1 ~ "/bombable/weapons-burst-count", 0);
    }
    var loopLength = 0.5 + rand() * 1.5;
  }
...
# Distance to player - fire escalation
var _dist_m = getprop(myNodeName1 ~ "/bombable/attributes/distances/target-distance-m");
if (_dist_m != nil) {
  if (_dist_m > 800) { _fireChance = 0.3; }
  elsif (_dist_m > 400) { _fireChance = 0.6; }
  if (rand() > _fireChance) return;
}
  ...
  foreach (elem;keys (weaps) ) {
    ...
    # Toggle fire-trigger on AI node based on weapon type
    # bombable-weapons-sound.xml plays the correct sound per category
    var _wname = weaps[elem].name;
    if (_wname == nil) _wname = "";
    var _trigger = "bombable/fire-trigger-mg"; # default
    foreach (var w; ["76mm","85mm","88mm","90mm","100mm","105mm","122mm","128mm","152mm","155mm","tank gun","Tank         Gun","shell","Shell","ZIS","KwK","PaK","F-34","howitzer","artillery"]) {
        if (find(w, _wname) != -1) { _trigger = "bombable/fire-trigger-shell"; break; }
    }
    foreach (var w; ["20 mm","20mm","23mm","30mm","37mm","40mm","cannon","Cannon","GAU-        8","M61","Hispano","MK108","MG151","ShVAK","Oerlikon","Bofors","SUU"]) {
        if (find(w, _wname) != -1) { _trigger = "bombable/fire-trigger-cannon"; break; }
    }
    foreach (var w; [".50",".50 BMG","50cal","12.7","12.7mm","13mm","Ho-103","MG131","M2","DShK","HMG"]) {
        if (find(w, _wname) != -1) { _trigger = "bombable/fire-trigger-heavy-mg"; break; }
    }
    foreach (var w; ["rocket","Rocket","HVAR","RP-3","Zuni","Hydra","S-5","S-8","LAU"]) {
        if (find(w, _wname) != -1) { _trigger = "bombable/fire-trigger-rocket"; break; }
    }
    foreach (var w; ["missile","Missile","AIM","SAM","Sidewinder","Sparrow","R-60","R-73"]) {
        if (find(w, _wname) != -1) { _trigger = "bombable/fire-trigger-missile"; break; }
    }
    foreach (var w; ["bomb","Bomb","MK-81","MK-82"]) {
        if (find(w, _wname) != -1) { _trigger = "bombable/fire-trigger-bomb"; break; }
    }
    foreach (var w; ["torpedo","Torpedo"]) {
        if (find(w, _wname) != -1) { _trigger = "bombable/fire-trigger-torpedo"; break; }
    }
    var _cur = getprop(myNodeName1 ~ "/" ~ _trigger) or 0;
    setprop(myNodeName1 ~ "/" ~ _trigger, !_cur);

    # Toggle impact-trigger on AI node with delay

    var _impact_trigger = nil;
    var _impact_delay = 0.8;

     if (_trigger == "bombable/fire-trigger-shell" or
      _trigger == "bombable/fire-trigger-cannon") {

      _impact_trigger = "/bombable/impact-trigger-shell";
      _impact_delay = 3.12;

    } elsif (_trigger == "bombable/fire-trigger-rocket" or
         _trigger == "bombable/fire-trigger-missile" or
         _trigger == "bombable/fire-trigger-torpedo" or
         _trigger == "bombable/fire-trigger-bomb") {

         _impact_trigger = "/bombable/impact-trigger-explosion";
         _impact_delay = 3.20;

    } else {

    _impact_trigger = "/bombable/impact-trigger-mg";
    _impact_delay = 0.9;
    }

    settimer(func {

    var _icur = getprop(myNodeName1 ~ _impact_trigger) or 0;
    setprop(myNodeName1 ~ _impact_trigger, !_icur);

    }, _impact_delay);

    # Ambient sound trigger - per weapon type
    var _ambient = myNodeName1 ~ "/bombable/ambient-trigger-" ~ substr(_trigger, size("bombable/fire-trigger-"));
    var _acur = getprop(_ambient) or 0;
    setprop(_ambient, !_acur);
...
# weapons_loop_ai - AI shoots at other AI of different faction
# Also handles max-ai-attack-player logic: limits how many AI
# simultaneously engage the human player based on skill setting.
var weapons_loop_ai = func (id, myNodeName1="") {

  var loopid = getprop(""~myNodeName1~"/bombable/loopids/weapons-ai-loopid");
  id == loopid or return;

  var loopLength = 2.0 + rand() * 3.0;
  settimer( func {
    weapons_loop_ai (id, myNodeName1);
  }, loopLength);

  if ( !getprop(bomb_menu_pp~"ai-vs-ai-enabled") ) return;
  ...
  # Use nearestEnemyAI which checks both AI faction and player faction
  # targeting-player flag is managed by attack_loop now - weapons_loop_ai
  # just needs to know who to shoot at
  var _enemy = nearestEnemyAI(myNodeName1);
  var bestTarget = _enemy.node;

  # If no AI enemy found, nothing for weapons_loop_ai to do
  # (attack_loop handles player targeting)
  if (bestTarget == nil) return;

  # Use existing weapons_loop mechanism with AI as target
  var targetSize_m = (contains(attributes, bestTarget) and contains(attributes[bestTarget], "size_m")) ? attributes[bestTarget].size_m : {horz:10, vert:5};

  var aiAimFudgeFactor = getprop(bomb_menu_pp~"ai-weapon-power") or 11.5;
  var pilotSkill = getprop(myNodeName1~"/bombable/weapons-pilot-ability") or 0;
  aiAimFudgeFactor += pilotSkill * 9;
  ...
  foreach (elem; keys(weaps)) {
    ...
    var result = checkAim(myNodeName1, bestTarget, targetSize_m, aiAimFudgeFactor,
                          weaps[elem].maxDamageDistance_m, weaps[elem].weaponAngle_deg,
                          weaps[elem].weaponOffset_m, 0);

    if (result == 0) continue;

    # Fire trigger - sound
    var _wname = weaps[elem].name or "";
    var _trigger = "bombable/fire-trigger-mg";
    foreach (var w; ["shell","Shell","76mm","85mm","88mm","105mm","122mm","152mm","howitzer","artillery"]) {
      if (find(w, _wname) != -1) { _trigger = "bombable/fire-trigger-shell"; break; }
    }
    foreach (var w; ["cannon","Cannon","20mm","23mm","30mm","37mm"]) {
      if (find(w, _wname) != -1) { _trigger = "bombable/fire-trigger-cannon"; break; }
    }
    foreach (var w; ["heavy-mg","Heavy-MG","heavy_mg",".50",".50 BMG","50cal","12.7","12.7mm","13mm","Ho-103","MG131","M2","DShK","HMG"]) {
      if (find(w, _wname) != -1) { _trigger = "bombable/fire-trigger-heavy-mg"; break; }
    }
    foreach (var w; ["rocket","Rocket","HVAR"]) {
      if (find(w, _wname) != -1) { _trigger = "bombable/fire-trigger-rocket"; break; }
    }
    var _cur = getprop(myNodeName1 ~ "/" ~ _trigger) or 0;
    setprop(myNodeName1 ~ "/" ~ _trigger, !_cur);
    ...
    # Damage at target AI node
    var r = rand();
    if (r < result) {
      var damageAdd = result * weaps[elem].maxDamage_percent / 100;
      var _tlat = getprop(bestTarget ~ "/position/latitude-deg");
      var _tlon = getprop(bestTarget ~ "/position/longitude-deg");
      var _talt = getprop(bestTarget ~ "/position/altitude-ft");
      if (_tlat != nil and _tlon != nil and _talt != nil) {
        add_damage(damageAdd, bestTarget, "weapons", nil, nil, _tlat, _tlon, _talt * FT2M);
      } else {
        add_damage(damageAdd, bestTarget, "weapons");
      }
    } elsif (result > 0.05) {
      # Near miss - targeted AI may still react and dodge even without a hit
      var _tSkill = calcPilotSkill(bestTarget);
      if ( rand() < (result * (0.3 + _tSkill * 0.1)) ) {
        debprint("Bombable: AI near-miss evasion triggered for ", bestTarget);
        dodge(bestTarget);
      }
    }

    stores.reduceWeaponsCount(myNodeName1, elem, loopLength);
  }
}
...
####################################################
# find nearest enemy for myNodeName - checks AI faction AND player faction
var nearestEnemyAI = func (myNodeName) {
  var myFaction     = (contains(attributes, myNodeName) and contains(attributes[myNodeName], "faction")) ? attributes[myNodeName].faction : "";
  var playerFaction = getprop("/bombable/player-faction") or "";
  var friendlyFireAllowed = getprop(bomb_menu_pp~"friendly-fire-allowed") or 0;

  # Default: attack unless proven friendly
  var playerIsEnemy = 1;
  if (!friendlyFireAllowed and myFaction != "" and playerFaction != "") {
    var myFactions     = split(",", myFaction);
    var playerFactions = split(",", playerFaction);
    ...
    foreach (var f; myFactions) {
      foreach (var pf; playerFactions) {
        if (string.trim(f) == string.trim(pf)) { friendly = 1; break; }
      }
      if (friendly) break;
    }
    playerIsEnemy = !friendly;
  }

  # Scan all relevant AI model types: aircraft, ship, multiplayer
  var targets = props.globals.getNode("/ai/models").getChildren("aircraft");
  foreach (var _s; props.globals.getNode("/ai/models").getChildren("ship")) {
    append(targets, _s);
  }
  foreach (var _m; props.globals.getNode("/ai/models").getChildren("multiplayer")) {
    append(targets, _m);
  }
  var bestNode = nil;
  var bestDist = 99999999;

  foreach (var t; targets) {
    var tName = t.getPath();
    if (tName == myNodeName) continue;
    if (!contains(attributes, tName)) continue;
    var tFaction = contains(attributes[tName], "faction") ? attributes[tName].faction : "";
    # if no faction on either side -- treat as enemy (backward compatible)
    if (tFaction == "" or myFaction == "") {
      var myPos = any_aircraft_position(myNodeName);
      var tPos  = any_aircraft_position(tName);
      if (myPos == nil or tPos == nil) continue;
      var d = myPos.direct_distance_to(tPos);
      if (d < bestDist) { bestDist = d; bestNode = tName; }
      continue;
    }
    # check if enemy faction
    var tFactions  = split(",", tFaction);
    var myFactions = split(",", myFaction);
    var isFriendly = 0;
    foreach (var f; myFactions) {
      foreach (var tf; tFactions) {
        if (string.trim(f) == string.trim(tf)) { isFriendly = 1; break; }
      }
      if (isFriendly) break;
    }
  ...
  return { node: bestNode, dist: bestDist, playerIsEnemy: playerIsEnemy };
}

####################################################
# returns [direct_dist_m, alt_diff_m] from myNodeName to targetNodeName
var distAItoTarget = func (myNodeName, targetNodeName) {
  var myPos = any_aircraft_position(myNodeName);
  var tPos  = any_aircraft_position(targetNodeName);
  return [ myPos.direct_distance_to(tPos), myPos.alt() - tPos.alt() ];
}

####################################################
# returns course from myNodeName to targetNodeName
var courseAItoTarget = func (myNodeName, targetNodeName) {
  var myPos = any_aircraft_position(myNodeName);
  var tPos  = any_aircraft_position(targetNodeName);
  return myPos.course_to(tPos);
}
...
var attack_loop = func (id, myNodeName, looptime) {
  ...
  # --- AI vs AI + Player targeting decision ---
  var _enemy        = nearestEnemyAI(myNodeName);
  var _aiTarget     = _enemy.node;
  var _aiDist       = _enemy.dist;
  var _playerIsEnemy = _enemy.playerIsEnemy;

  var _targetingPlayer = getprop(""~myNodeName~"/bombable/targeting-player") or 0;

  # If player is not an enemy faction, never target player
  if (!_playerIsEnemy) {
    _targetingPlayer = 0;
    setprop(""~myNodeName~"/bombable/targeting-player", 0);
  }

# If targeting player, check max-ai-attack-player cap
  if (_playerIsEnemy) {
    var maxAttackPlayer = getprop(bomb_menu_pp~"max-ai-attack-player") or 3;
    # 0 means unlimited
    if (maxAttackPlayer == 0) {
      _targetingPlayer = 1;
    } else {
      var attackingPlayerCount = 0;
      var allAI = props.globals.getNode("/ai/models").getChildren("aircraft");
      foreach (var _s2; props.globals.getNode("/ai/models").getChildren("ship")) {
        append(allAI, _s2);
      }
      foreach (var _m2; props.globals.getNode("/ai/models").getChildren("multiplayer")) {
        append(allAI, _m2);
      }
      foreach (var _a; allAI) {
        var _aName = _a.getPath();
        if (_aName == myNodeName) continue;
        if (getprop(_aName~"/bombable/targeting-player") == 1) attackingPlayerCount += 1;
      }
      if (attackingPlayerCount < maxAttackPlayer) {
        _targetingPlayer = 1;  # under cap - attack player
      } else {
        _targetingPlayer = 0;  # cap reached - attack AI instead
      }
    }
    setprop(""~myNodeName~"/bombable/targeting-player", _targetingPlayer);
  }

  # If no AI enemy and player is not enemy either - nothing to do
  if (_aiTarget == nil and !_playerIsEnemy) return;

  # Pick final target
  var _useAITarget = (!_targetingPlayer and _aiTarget != nil);
  setprop(""~myNodeName~"/bombable/ai-attack-target", _useAITarget ? _aiTarget : "");

  var dist               = _useAITarget ? distAItoTarget(myNodeName, _aiTarget)   : distAItoMainAircraft(myNodeName);
  var courseToTarget_deg = _useAITarget ? courseAItoTarget(myNodeName, _aiTarget) : courseToMainAircraft(myNodeName);
  ...
  var targetHeading_deg = _useAITarget ? (getprop(""~_aiTarget~"/orientation/true-heading-deg") or 0) : getprop("/orientation/heading-deg");
...
if (damagetype=="weapon" or damagetype=="weapons") {
    ...
    # Draw flack-impact visual on AI model when hit by weapons, if we have a valid position
    # and the flack effect is enabled. This mirrors the MP model behavior.
    if ( damageRise > 0 and lat_deg != nil and lon_deg != nil and alt_m != nil
          and getprop("/bombable/menusettings/fire-particles/flack-trigger") ) {
      var ai_startSize_m = nil;
      var ai_endSize_m   = nil;
      if ( ballisticMass_lb != nil ) {
        if (ballisticMass_lb < 1.2) {
          ai_startSize_m = 0.25 + ballisticMass_lb/3;
          ai_endSize_m   = 1 + ballisticMass_lb;
        } else {
          ai_startSize_m = 0.25 + ballisticMass_lb/1000;
          ai_endSize_m   = 2 + ballisticMass_lb/4;
        }
      }
      debprint("Bombable: Drawing flack-impact on AI model at ", lat_deg, " ", lon_deg, " ", alt_m);
      var _offset = 3;
      var _dlat = (rand() - 0.5) * 2 * _offset / 111320;
      var _dlon = (rand() - 0.5) * 2 * _offset / 111320;
      var _dalt = rand() * _offset;
      put_remove_model( lat_deg + _dlat, lon_deg + _dlon, alt_m + _dalt, 0.75, ai_startSize_m, ai_endSize_m,
                        "AI/Aircraft/Fire-Particles/smallflack.xml" );
    }
  }
  ...
  # AI evasion when hit by weapons (AI vs AI or player vs AI)
  # This triggers dodge maneuver on the hit AI object so it tries to evade
  if ( damagetype == "weapons" and damageRise > 0 and type != "multiplayer" and myNodeName != "" ) {
    var hitSkill = calcPilotSkill(myNodeName);
    # More skilled pilots react faster and more reliably to being hit
    if ( rand() < (0.4 + hitSkill * 0.12) ) {
      debprint("Bombable: AI hit by weapons, triggering evasion dodge for ", myNodeName);
      dodge(myNodeName);
    }
  }
...
var weapons_init_func = func(myNodeName) {
  ...
  # init targeting-player flag (used by max-ai-attack-player logic)
  setprop(""~myNodeName~"/bombable/targeting-player", 0);
  ...
  if ( type != "multiplayer" ) {
    ...
    # Start AI vs AI loop
    var loopid_ai = inc_loopid (myNodeName, "weapons-ai");
    settimer( func {
      weapons_loop_ai (loopid_ai, myNodeName);
    }, 7 + rand() * 5);
  }
